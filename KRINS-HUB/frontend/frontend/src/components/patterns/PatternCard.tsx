...
interface PatternCardProps {
  ...
  className?: string;
  variant?: 'default' | 'netflix' | 'compact';
...
export default function PatternCard({
  ...
  // Netflix variant - horizontal card with thumbnail and actions
  if (variant === 'netflix') {
    return (
      <div
        className={cn(
          ...
          "h-48", // Fixed height for Netflix-style consistency
          className
        )}
        onClick={handleClick}
      >
        ...
        <div className="absolute inset-0 p-4 flex flex-col justify-between">
          ...
          <div className="flex items-start justify-between">
            <div className="space-y-1">
              ...
              {pattern.difficulty && (
                <div className={cn(
                  ...
                  pattern.difficulty === 'beginner' && "bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300",
                  pattern.difficulty === 'intermediate' && "bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-300",
                  pattern.difficulty === 'advanced' && "bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300"
                )}>
                  {pattern.difficulty}
                </div>
              )}
            ...
            <Button
              variant="ghost"
              size="icon"
              className="opacity-0 group-hover:opacity-100 transition-opacity bg-background/50 backdrop-blur"
              onClick={(e) => {
                e.stopPropagation();
                onBookmark?.();
              }}
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center space-x-4 text-xs">
              ...
              <div className="flex items-center space-x-1">
                ...
                <span className="text-muted-foreground">{pattern.usage_count}</span>
              </div>
            </div>
  ...
  if (variant === 'compact') {
    return (
      <div
        onClick={handleClick}
        className={cn(
          "group flex items-center space-x-3 p-3 border border-border rounded-lg bg-card",
          "hover:shadow-md transition-all duration-200 cursor-pointer hover:border-primary/30",
          className
        )}
      >
        <div className="flex-shrink-0 p-2 bg-yellow-100 dark:bg-yellow-900/20 rounded">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center space-x-2 mt-1">
            ...
            {pattern.effectiveness_score && (
              <div className="flex items-center space-x-1">
                <Star className="h-3 w-3 text-yellow-500 fill-current" />
                <span className="text-xs">{pattern.effectiveness_score}</span>
              </div>
            )}
          </div>
        </div>
