/**
 * 🧠 AI Personality Matcher - Test System
 * 
 * Hjelper brukere å finne sin perfekte AI Personal Companion
 * gjennom en intelligent personlighetstest
 * 
 * @author Krin & Team
 */
...
class PersonalityTest {
  ...
  /**
   * Calculate personality match based on user answers
   */
  calculateMatch(answers) {
    ...
    // Sum up trait scores from all answers
    ...
    Object.keys(this.personalityProfiles).forEach(personalityKey => {
      ...
      Object.keys(personality.traits).forEach(trait => {
        ...
        matchScore += similarity * personalityScore; // Weight by personality trait strength
        totalTraits += 3;
      });
  ...
  /**
   * Get formatted test questions for CLI/web display
   */
