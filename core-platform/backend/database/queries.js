...
class DatabaseQueries {
    ...
    async upsertPattern(pattern, embedding) {
        ...
        const embeddingText = [
            ...
            when_to_use,
            when_not_to_use,
            Array.isArray(context_tags) ? context_tags.join(' ') : context_tags,
            security_considerations
        ].filter(Boolean).join(' ');
    ...
    async getOrCreateUser(userData) {
        ...
        // Create new user with a default hash for system users
        const finalPasswordHash = password_hash || '$2b$12$dummy_hash_for_system_users';
        ...
        const result = await this.db.query(insertQuery, [username, email, finalPasswordHash]);
        return result.rows[0];
    }
}
